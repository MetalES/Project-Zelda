...
-- Initialize the values, when the script call the script
...
function tone_manager:on_started()
  ...
  if game.sun_song_finished then
    self:store_all_tone()
    game.sun_song_finished = false
  end
  -- Rebuild the Tone
  self:rebuild_tone()
end
...
local need_rebuild = false
  ...
  if game:get_map():get_tileset() ~= "hyrule" then
  ...
  if game.has_played_sun_song then
	...
	game:set_value("cr", tr)
	game:set_value("cg", tg)
	game:set_value("cb", tb)
    game:set_value("ca", ta)
	game:set_value("tr", tr)
    game:set_value("tg", tg)
    game:set_value("tb", tb)
    game:set_value("ta", ta)
  ...
  if hour == 24 then
   self:set_new_tone(0, 0, 64,160)
	...
	elseif hour == 4 and minute >= 30 then
      game:set_value("time_of_day", "night_ending")
	...
	elseif hour == 16 and minute == 0 and minute <= 30 then
      game:set_value("time_of_day", "day_ending")
      self:set_new_tone(253, 125, 5, 60)
