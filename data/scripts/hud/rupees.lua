...
function rupees:check()
  ...
  if money ~= self.money_displayed then
    ...
    if self.money_displayed == money then
      sol.audio.play_sound("objects/rupee/rupee_counter_end")

    -- While the counter is scrolling, play a sound every 3 values.
    elseif self.money_displayed % 2 == 0 then
      sol.audio.play_sound("objects/rupee/rupee_counter_end")
    end
  end
...
function rupees:rebuild_surface()
  ...
  -- Max money (icon).
  self.rupee_icons_img:draw_region((self.rupee_bag_displayed - 1) * 17, 0, 17, 12, self.surface)
