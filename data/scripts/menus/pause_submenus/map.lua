  ...
  if self.dungeon == nil then
    ...
    if self.hero_floor == nil then
      ...
      elseif self.hero_floor >= self.dungeon.highest_floor - 2 then
        self.highest_floor_displayed = self.dungeon.highest_floor -- - 1
    ...
    self.dungeon_map_spr = sol.sprite.create("menus/dungeon_maps/".. hero_mode .. "map" .. self.dungeon_index)
    self.dummy_dungeon_map_spr = sol.sprite.create("menus/dungeon_maps/".. hero_mode .. "map" .. self.dungeon_index)
...
function map_submenu:load_dungeon_map_image()
  ...
  if self.game:has_dungeon_compass() then
    ...
    for _, chest in ipairs(self.dungeon.chests) do
      if chest.floor == self.selected_floor and chest.savegame_variable ~= nil and not self.game:get_value(chest.savegame_variable) then
