...
function inventory_submenu:set_cursor_position(row, column)
  ...
  self.game.hud.primary.item_icon_1.surface:set_opacity(item_icon_opacity)
  self.game.hud.primary.item_icon_2.surface:set_opacity(item_icon_opacity)
end
    ...
    for i = 1, 2 do
	  if command == "item_" .. i then
	    if self:is_item_selected() then
		  if item_names[self:get_selected_index() + 1] == "spoils_bag" then
		    sol.menu.start(self, self.game.pause.submenus[8], false)
		    handled = true
		    return
		  end
		  self:assign_item(i)
...
function inventory_submenu:on_draw(dst_surface)
      ...
      if item_names[k] ~= nil then
        ...
        if item ~= nil and item:get_variant() > 0 then
          ...
          if self.counters[k] ~= nil then
            self.counters[k]:draw(dst_surface, x + 6, y - 1) --8
          end
        end
      end
      x = x + 32
  ...
  -- Draw the cursor.
  if not sol.menu.is_started(self.game.pause.submenus[8]) then
