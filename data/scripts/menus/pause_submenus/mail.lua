...
function mail_submenu:on_started()
  ...
  self.game:set_custom_command_effect("action", self.game:get_value("unread_mail_amount") ~= nil and "open" or nil)
...
function mail_submenu:on_finished()
  if sol.menu.is_started(self.game.pause.submenus[3]) then
    self.game.pause.submenus[3]:on_started()
  end
end
...
function mail_submenu:on_draw(dst_surface)
  ...
  -- Arrows.
  if self.game:get_value("unread_mail_amount") ~= nil then
...
function mail_submenu:on_command_pressed(command)
  ...
  if command == "pause" or self.game:is_dialog_enabled() then
	if not self.is_reading then
	  self.is_reading = false
	  sol.menu.stop(self)
	  return false
  ...
  if self.game:get_value("unread_mail_amount") == nil then return true end
