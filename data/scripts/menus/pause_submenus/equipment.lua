...
function equipment_submenu:on_started()
  ...
  for i = 1, 3 do
    if self.game:get_value("tunic_".. i .."_obtained") then
	  item_sprite:set_direction(i - 1)
	  item_sprite:draw(self.equipment_surface, 185 + (24 * i), 180)
	  self.caption_text_keys[4 + i] = "equipment.caption.tunic_" .. i
	end
  end
...
function equipment_submenu:on_command_pressed(command)
  ...
  if not handled then
	...
	elseif command == "action" then
	  ...
	  elseif self.cursor_position == 5 or self.cursor_position == 6 and self.game:get_value("tunic_2_obtained") or self.cursor_position == 7 and self.game:get_value("tunic_3_obtained") then
	    local target = self.cursor_position - 4
	    self.game:set_ability("tunic", target)
		self.game:get_hero():set_tunic_sprite_id("hero/tunic" .. target)
	    sol.audio.play_sound("/menu/select")
