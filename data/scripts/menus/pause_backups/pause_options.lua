...
local file = sol.game.load("system_boot.dat")
...
function options_submenu:on_command_pressed(command)
  ...
  if not handled then
    ...
    elseif command == "up" then
      ...
      self:set_cursor_position((self.cursor_position + 9) % 11 + 1)
      handled = true
    elseif command == "down" then
      sol.audio.play_sound("/menu/cursor")
      self:set_cursor_position(self.cursor_position % 11 + 1)
      handled = true
