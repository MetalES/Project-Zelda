...
local function on_collision(torch, other, torch_sprite, other_sprite)
  ...
  if other:get_type() == "custom_entity" then
    ...
    elseif other_model == "lamp_fire" then
	if not torch:is_lit() then
	  torch:get_game():get_hero():set_animation("lantern_swing", function()
	  torch:set_lit(true)
	  if torch.on_lit ~= nil then
          torch:on_lit()
      end 
	  local x, y, layer = torch:get_position()
	  local torch_burst = torch:get_game():get_map():create_custom_entity({
				x = x - 4,
				y = y - 10,
				layer = layer,
				direction = 0,
		...
		sol.timer.start(300, function() torch_burst:remove() end)	  
		torch:get_game():get_hero():set_animation("stopped")
		end) 
	  end
...
torch:set_traversable_by("custom_entity", function(torch, other)
  return other:get_model() == "fire_beam" or other:get_model() == "ice_beam" or other:get_model() == "lamp_fire"
end)
