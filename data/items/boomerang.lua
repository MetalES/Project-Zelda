...
local boomerang_sync
local boomerang_timer
...
function item:on_using()
  ...
  local function end_by_stairs() hero:set_walking_speed(88); game:set_custom_command_effect("attack", nil); game:set_ability("sword", game:get_value("item_saved_sword")); game:set_ability("shield", game:get_value("item_saved_shield")); if show_bars == true and game:get_value("starting_cutscene") ~= true then game:hide_bars() end; item:set_finished(); game:set_pause_allowed(true) end
  ...
  if state == 0 then 
	...
	sol.timer.start(60, function()	
		...
		hero:unfreeze()
		state = 1
	    hero:set_tunic_sprite_id("hero/item/boomerang/boomerang_moving_free_tunic"..tunic)
...
-- this is just a placeholder until the function will be back
			...
			if hero:get_state() == "hurt" then is_finished = true; hero:set_tunic_sprite_id("hero/tunic"..tunic); hero:start_hurt(0,0,0); end_by_collision() end
			if hero:get_state() == "jumping" or hero:get_state() == "swimming" then is_finished = true; hero:set_tunic_sprite_id("hero/tunic"..tunic); hero:set_position(lx + new_x, ly + new_y); end_by_collision() end
			if hero:get_state() == "falling" then is_finished = true; hero:set_tunic_sprite_id("hero/tunic"..tunic); end_by_collision() end
			if hero:get_state() == "treasure" then is_finished = true; hero:set_tunic_sprite_id("hero/tunic"..tunic); end_by_pickable() end
			if hero:get_state() == "stairs" then is_finished = true; hero:unfreeze(); hero:set_tunic_sprite_id("hero/tunic"..tunic); end_by_stairs() end
...
elseif state == 1 then
	if game:is_command_pressed(slot) and game:get_value("starting_cutscene") ~= true then 
		...
		hero:set_animation("boomerang_intro", function()
		sol.audio.play_sound("common/item_show") -- sol.timer.start(40, function()
		hero:unfreeze()
...
boomerang_timer = sol.timer.start(10, function()
	if not game:is_command_pressed(slot) and can_shoot and not avoid_return and game:get_value("starting_cutscene") ~= true then
		hero:set_walking_speed(50)
		if boomerang_timer ~= nil then boomerang_timer:stop() boomerang_timer = nil
			if not launched then
			hero:freeze()
			hero:set_tunic_sprite_id("hero/tunic"..tunic)
			hero:set_animation("boomerang_shoot", function()
...
if hero:get_state() ~= "swimming" and hero:get_state() ~= "sword tapping" and hero:get_state() ~= "sword spin attack" and hero:get_state() ~= "sword swinging" and hero:get_state() ~= "sword loading" and hero:get_state() ~= "carrying" and hero:get_state() ~= "lifting" and hero:get_state() ~= "hurt" and hero:get_state() ~= "jumping" and hero:get_state() ~= "falling" and hero:get_state() ~= "treasure" and hero:get_state() ~= "stairs" and game:get_value("starting_cutscene") ~= true and game:get_value("using_item") ~= true and hero:get_tunic_sprite_id() == "hero/tunic"..tunic then hero:freeze(); hero:set_animation("boomerang_catch"); sol.audio.play_sound("characters/link/voice/catch_boomerang") end
...
sol.timer.start(120, function()
	if not is_finished then 
		...
		launched = false
		avoid_return = false
	    if hero:get_state() ~= "swimming" and hero:get_state() ~= "carrying" and hero:get_state() ~= "sword tapping" and hero:get_state() ~= "sword spin attack" and hero:get_state() ~= "sword swinging" and hero:get_state() ~= "sword loading" and hero:get_state() ~= "lifting" and hero:get_state() ~= "hurt" and hero:get_state() ~= "jumping" and hero:get_state() ~= "falling" and hero:get_state() ~= "treasure" and hero:get_state() ~= "stairs" and game:get_value("starting_cutscene") ~= true and game:get_value("using_item") ~= true and hero:get_tunic_sprite_id() == "hero/tunic"..tunic then hero:unfreeze() end
		item:set_finished()
    end
sol.audio.play_sound("common/item_show")
end)
